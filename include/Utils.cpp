...
namespace DebugAPI_IMPL
{
	...
	glm::vec2 DebugAPI::WorldToScreenLoc(RE::GPtr<RE::GFxMovieView> movie, glm::vec3 worldLoc)
	{
		...
		RE::NiCamera::WorldPtToScreenPt3((float(*)[4])g_worldToCamMatrix, *g_viewPort, niWorldLoc, screenLocOut.x, screenLocOut.y, zVal, 1e-5f);
...
namespace FenixUtils
{
	...
	RE::Effect* ActorHasEffectWithKeyword(RE::Actor* a_actor, RE::FormID a_formID)
	{
	...
	{
		if (auto actor = a->As<RE::Actor>()) {
			return ActorHasEffectWithKeyword(actor, kwd->formID);
		}
